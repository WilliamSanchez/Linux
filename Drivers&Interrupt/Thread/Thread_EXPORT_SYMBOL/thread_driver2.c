...
static ssize_t w_read(struct file *filp, char __user *buf, size_t len, loff_t *off){
  ...
  W_shared_func();
  pr_info("%d time(s) sahred function called!\n",W_count);
  pr_info("Data read: Done !\n");
  return 0;
}
...
static int __init first_driver_init(void){
  ...
  if((alloc_chrdev_region(&dev,0,1,"WilliamSanchez_Driver[2]")) <0 ){
   ...
   if((dev_class = class_create(THIS_MODULE,"William_class[2]")) == NULL){
	pr_err("cannot add the deviceto the system\n");
 	goto r_class;
   }
 
   /*create device*/
   if((device_create(dev_class,NULL,dev,NULL,"William_device[2]"))==NULL){
