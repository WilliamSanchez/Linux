...
static DEFINE_RWLOCK(W_spinlock);
unsigned long W_global_variable = 0;

//Dynamic method to initialize the read and write spinlock
// rwlock_t W_spinlock;
...
int thread_function1(void *pv){
   ...
   while(!kthread_should_stop()){
	write_lock(&W_spinlock);
	W_global_variable++;
	write_unlock(&W_spinlock);
	msleep(1000);
   }
   return 0;
}
...
int thread_function2(void *pv){
   ...
   while(!kthread_should_stop()){
	read_lock(&W_spinlock);
	pr_info("William thread function [2]... %lu\n",W_global_variable);
	read_unlock(&W_spinlock);
	msleep(1000);
   }
   return 0;
}
...
//Dynamic methot to initialize the read write spinlock
//rwlock_init(&W_spinlock);    
