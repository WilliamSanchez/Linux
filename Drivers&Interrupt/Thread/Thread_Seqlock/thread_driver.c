...
int thread_function1(void *pv){
   ...
   while(!kthread_should_stop()){
	write_seqlock(&W_seq_lock);
	W_global_variable++;
        write_sequnlock(&W_seq_lock);
	msleep(1000);
   }
   return 0;
}
...
int thread_function2(void *pv){
   ...
   while(!kthread_should_stop()){
	do{
	  seq_no = read_seqbegin(&W_seq_lock);
	  read_value = W_global_variable;
	}while(read_seqretry(&W_seq_lock,seq_no));
	pr_info("William Sanchez Thread function [2]: read value %lu\n", read_value);
 	msleep(1000);
   }
   return 0;
}
