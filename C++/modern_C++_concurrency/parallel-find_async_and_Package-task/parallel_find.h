...
Iterator parallel_find_pt(Iterator first, Iterator last, MatchType match)
{
    ...
    /*  Calculatr the optimized number of threads to run the algorithm  */
...
Iterator parallel_find_async(Iterator first, Iterator last, MatchType match, std::atomic<bool>* done_flag)
{
    try
    {
        ...
        {   
            ...
            std::future<Iterator> async_result = std::async(&parallel_find_async(first, mid_point,match,done_flag));

            std::future<Iterator> direct_result = parallel_find_async(first, mid_point, match, done_flag);
